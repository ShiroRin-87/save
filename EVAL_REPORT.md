...
| 指标 | A | B | C | D |
|------|---|---|---|---|
| 准确率 | 5/10 (50%) | 6/10 (60%) | 7/10 (70%) | 5/9 (56%) |
| 幻觉率 | 高 | 中 | 低 | 极低 |
| 平均耗时 | ~30s | ~90s | ~250s | ~400s |
...
| D | ~250K | ~5K | 8-10 | +claims提取(并行×30页)+验证 |
| E | ~260K | ~6K | 8-10 | +fallback约束拆解(仅在D失败时) |

**D/E的claims提取是主要成本来源**——30页×每页5-15K字符送入LLM，输入token急剧膨胀。

## 4. 召回率分析

召回率定义为：搜索结果中是否包含正确答案的相关信息。

| 搜索策略 | 召回率 | 典型场景 |
|----------|--------|----------|
| 谜面原文搜索 | ~30% | 谜面与答案零关键词重叠 |
| Gap-fill补充 | +20% | 识别中间实体后定向搜索 |
| 假设驱动搜索 | +30% | LLM参数知识桥接谜面→答案 |
| **组合后总召回** | **~80%** | 三者互补 |

**关键瓶颈**：SerpAPI/Google通用网页搜索无法覆盖：
- 学术论文元数据（CNKI/万方专有）
- 低热度中文网络内容
- 需要结构化查询的信息
...
| P2 | 链式推理（A→B确认后B→C）替代一次性生成 | 解决BC007类4+跳问题 |
