...
JSON数组："""
    ...
    if reasoning:
        ...
        key_lines = [l.strip() for l in lines if any(kw in l for kw in ['线索','推断','可能是','确定','结论','答案','所以','综上','推测','得出','认为','关键'])]
        for l in key_lines[:8]:
            if len(l) > 15:
                print(f"      {l[:200]}")
    result = {"question": question, "ref": ref, "versions": {"V0": {"hypotheses": hyps, "reasoning": reasoning, "hit": hit}}}
